...
type StateColorsType = ({
    color: ProductColorsType;
    isSelected: boolean;
    urlImage: string;
})[]

type SizesProductType = { stateColors: StateColorsType, changeStateColors: React.Dispatch<React.SetStateAction<StateColorsType>> }
...
export default function RadioColorSwitcher({ stateColors, changeStateColors }: SizesProductType) {
    ...
    function onToggleColor(pressedColor: string) {
        ...
        const isPressedColorActive = rawColorsObject.some((element) => element.color === pressedColor && element.isSelected);
